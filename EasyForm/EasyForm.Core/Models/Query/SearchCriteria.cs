...
namespace EasyForm.Core.Models.Query
{
    ...
    {
        public SearchCriteria(int pageNo, int pageSize, IEnumerable<IRecordSorter> sorters)
        : this(pageNo, pageSize, null, sorters)
        {
        }

        public SearchCriteria(int pageNo, int pageSize, IRecordFilter filter, IRecordSorter sorter)
           : this(pageNo, pageSize, filter, new List<IRecordSorter> { sorter })
        {
        }

        public SearchCriteria(int pageNo, int pageSize, IRecordSorter sorter)
            : this(pageNo, pageSize, null, new List<IRecordSorter> { sorter })
        {
        }

        public SearchCriteria(int pageNo,int pageSize,IRecordFilter filter, IEnumerable<IRecordSorter> sorters)
        {
            if (pageNo < 1)
            {
